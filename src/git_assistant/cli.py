...
from git_assistant.changelog.writer import get_changelog_path
from git_assistant.release.evaluator import evaluate_release
from git_assistant.release.ai_evaluator import evaluate_release_with_ai, AIReleaseSuggestion
...
def print_ai_release_suggestion(cwd: Path, ai_config: AIConfig) -> None:
    """
    Evaluate and print an AI-based release suggestion.
    ...
    try:
        suggestion = evaluate_release_with_ai(changelog_path, ai_config)
    except (AIProviderError, ValueError) as exc:
        if ai_config.debug:
            print(f"\n[DEBUG] AI release evaluation failed: {exc}")
        return

    if not ai_config.debug and not suggestion.should_release:
        return
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print_ai_release_suggestion(cwd, ai_config=ai_config)
